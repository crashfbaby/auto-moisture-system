...
PLANT_PROFLIES = 'plant.dat'

#save plant info to 'plant.dat'     
def write_plant_profiles():  
    lines = []
    for plant in PLANTS: # PLANTS needs to be global list of plant objects so it writes entire plant profile for each plant. Use create_plant_objects() to make list
        lines.append("%s;%s,%s,%s,%s,%s,%s\n" % (plant.id, plant.moisture_pin, plant.pump_pin, plant.desired_moisture_percent, plant.moisture_min, plant.moisture_max, plant.name))
...
def read_plant_profiles():
    ...
    for line in lines:
        ...
        plant_info_dict['desired_moisture_percent'] = plant_info_list[2]
        plant_info_dict['moisture_min'] = plant_info_list[3]
        plant_info_dict['moisture_max'] = plant_info_list[4]
...
class Plant :
    ...
    def update_desired_moisture_percent(self):
...
def create_plant_objects():
    ...
    plants = {}
    if plant_profiles == None:  #create new plants if no plant.dat file exists
        for i in range(len(pump_pins)):
            new_plant = Plant(str(i), moisture_pins[i], pump_pins[i], plant_profiles[str(i)])
        ...
        for i in range(len(plant_profiles)): 
            ...
            existing_plant = Plant(str(i), plant_info['moisture_pin'], plant_info['pump_pin'])
            existing_plant.desired_moisture_percent = plant_info['desired_moisture_percent']    
            existing_plant.moisture_min = plant_info['moisture_min']      
            existing_plant.moisture_max = plant_info['moisture_max']   
